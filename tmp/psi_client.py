...
def _service_account_token() -> str:
    ...
    # Ensure we request an OAuth access token rather than a self-signed JWT.
...
def _extract_insights(audits: dict) -> List[Dict[str, object]]:
    ...
    for audit_id, audit in audits.items():
        ...
        if isinstance(details, dict) and details.get("type") == "opportunity":
            # Already captured as an improvement.
            continue
    ...
    # Keep the list concise – the most actionable insights tend to have a score (binary informative)
